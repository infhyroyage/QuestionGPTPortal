...
import TestResultAccordionUnit from "./TestResultAccordionUnit";
    ...
    order && (
      <div>
        {histories.map((history: History, historyIdx: number) => (
          <TestResultAccordionUnit
            key={historyIdx}
            getQuestion={getQuestions[`${historyIdx}`]}
            history={history}
            historyIdx={historyIdx}
            questionNumber={`${order[historyIdx]}`}
            isFavorite={!!favorites && favorites[historyIdx]}
            isOpen={openHistoryIdxes.includes(`${historyIdx}`)}
            isLoadingFavoriteButton={favorites === undefined}
