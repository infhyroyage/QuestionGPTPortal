...
- **Azurite の API バージョン**: `azure-storage-queue` 12.17.0 以降の既定 REST API (`2026-06-06`) は Azurite 3.36 時点で未対応。`compose.yaml` の Azurite に `--skipApiVersionCheck` を付与し、ローカルの `QueueClient` は `AZURITE_COMPATIBLE_API_VERSION` (`2025-11-05`) を明示する。未設定のままだと `import_local.py` の `create_queue_storages()` が `HttpResponseError: The API version 2026-06-06 is not supported by Azurite` で失敗する。
